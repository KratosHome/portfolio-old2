import react from '@/assets/technologies/ReactLogo.svg'

export const projectsUa: any = [
  {
    id: 1,
    isEmptiness: true,
  },
  {
    id: 2,
    count: '01',
    isEmptiness: false,
    title: 'платформа для профісионалів та замовників',
    company: 'Rastcom',
    description:
      'Rastcom перетворює спосіб, яким клієнти та професіонали знаходять один одного. Від юристів до офіціантів, наша платформа пропонує інтуїтивний календар, персоналізовані профілі, інтегрований чат, зручне відстеження замовлень та гнучке укладення контрактів, контроль робіт адміністраторами сайту за допомогою адмін панелі. З Rastcom, кожна послуга - це лише кілька кліків.',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    technologies2:
      'react, TypeScript, mui, Redux Toolkit,  React Router, Socket.io, Braintree, React Big Calendar, Git',
    link: 'https://www.rastcom.ca',
  },
  {
    id: 3,
    count: '02',
    isEmptiness: false,
    title: 'мобільний застосунок для профісионалів та замовників',
    company: 'Rastcom',
    description:
      "Додаток Rastcom - це ваш шлях до швидкого і надійного з'єднання з професіоналами у будь-якій сфері. Він забезпечує легке управління замовленнями, гнучке планування та безперервне спілкування, все в одному додатку, який завжди з вами.",
    technologies2: 'React Native, TypeScript, Redux Toolkit, git',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    link: null,
  },
  {
    id: 4,
    count: '03',
    isEmptiness: false,
    title: 'meta data index',
    description:
      "Частина системи Etere Media Asset Management, яка оркеструє робочі процеси та оптимізує цінність активів через централізоване управління медіа-контентом та пов'язаними метаданими",
    link: 'https://www.etere.com/DocView/9710/Etere-Launches-Web-Application-for-the-Fast-Insertion-of-Metadata.aspx',
    technologies2: 'React, ElectronJS, SCSS, MobX, SOAP, XML, SVN',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
  },
  {
    id: 5,
    count: '03',
    isEmptiness: false,
    title: 'e-commerce косметики та парфюмерії',
    company: 'Sharm Beauty',
    description:
      'Як ментор та тім лід, я веду студентську команду у процесі розробки та модернізації вебсайту https://sharmbeauty.ua, забезпечуючи інтеграцію найкращих практик у дизайні, SEO та розробці за допомогою NextJS та React.',
    technologies2: 'react, NextJs, Redux Toolkit, framer motion, GSAP. Git',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    link: null,
  },
  {
    id: 6,
    isEmptiness: true,
  },
  {
    id: 7,
    isEmptiness: true,
  },
  {
    id: 8,
    count: '04',
    isEmptiness: false,
    title: 'платформа для профісионалів та замовників',
    company: 'Rastcom',
    description:
      'Rastcom перетворює спосіб, яким клієнти та професіонали знаходять один одного. Від юристів до офіціантів, наша платформа пропонує інтуїтивний календар, персоналізовані профілі, інтегрований чат, зручне відстеження замовлень та гнучке укладення контрактів, контроль робіт адміністраторами сайту за допомогою адмін панелі. З Rastcom, кожна послуга - це лише кілька кліків.',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    technologies2:
      'react, TypeScript, mui, Redux Toolkit,  React Router, Socket.io, Braintree, React Big Calendar, Git',
    link: 'https://www.rastcom.ca',
  },
  {
    id: 9,
    count: '05',
    isEmptiness: false,
    title: 'мобільний застосунок для профісионалів та замовників',
    company: 'Rastcom',
    description:
      "Додаток Rastcom - це ваш шлях до швидкого і надійного з'єднання з професіоналами у будь-якій сфері. Він забезпечує легке управління замовленнями, гнучке планування та безперервне спілкування, все в одному додатку, який завжди з вами.",
    technologies2: 'React Native, TypeScript, Redux Toolkit, git',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    link: null,
  },
  {
    id: 10,
    count: '06',
    isEmptiness: false,
    title: 'meta data index',
    description:
      "Частина системи Etere Media Asset Management, яка оркеструє робочі процеси та оптимізує цінність активів через централізоване управління медіа-контентом та пов'язаними метаданими",
    link: 'https://www.etere.com/DocView/9710/Etere-Launches-Web-Application-for-the-Fast-Insertion-of-Metadata.aspx',
    technologies2: 'React, ElectronJS, SCSS, MobX, SOAP, XML, SVN',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
  },
  {
    id: 11,
    count: '07',
    isEmptiness: false,
    title: 'e-commerce косметики та парфюмерії',
    company: 'Sharm Beauty',
    description:
      'Як ментор та тім лід, я веду студентську команду у процесі розробки та модернізації вебсайту https://sharmbeauty.ua, забезпечуючи інтеграцію найкращих практик у дизайні, SEO та розробці за допомогою NextJS та React.',
    technologies2: 'react, NextJs, Redux Toolkit, framer motion, GSAP. Git',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    link: null,
  },
  {
    id: 12,
    isEmptiness: true,
  },
  {
    id: 13,
    isEmptiness: true,
  },
  {
    id: 14,
    count: '08',
    isEmptiness: false,
    title: 'платформа для профісионалів та замовників',
    company: 'Rastcom',
    description:
      'Rastcom перетворює спосіб, яким клієнти та професіонали знаходять один одного. Від юристів до офіціантів, наша платформа пропонує інтуїтивний календар, персоналізовані профілі, інтегрований чат, зручне відстеження замовлень та гнучке укладення контрактів, контроль робіт адміністраторами сайту за допомогою адмін панелі. З Rastcom, кожна послуга - це лише кілька кліків.',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    technologies2:
      'react, TypeScript, mui, Redux Toolkit,  React Router, Socket.io, Braintree, React Big Calendar, Git',
    link: 'https://www.rastcom.ca',
  },
  {
    id: 15,
    count: '09',
    isEmptiness: false,
    title: 'мобільний застосунок для профісионалів та замовників',
    company: 'Rastcom',
    description:
      "Додаток Rastcom - це ваш шлях до швидкого і надійного з'єднання з професіоналами у будь-якій сфері. Він забезпечує легке управління замовленнями, гнучке планування та безперервне спілкування, все в одному додатку, який завжди з вами.",
    technologies2: 'React Native, TypeScript, Redux Toolkit, git',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    link: null,
  },
  {
    id: 16,
    count: '10',
    isEmptiness: false,
    title: 'meta data index',
    description:
      "Частина системи Etere Media Asset Management, яка оркеструє робочі процеси та оптимізує цінність активів через централізоване управління медіа-контентом та пов'язаними метаданими",
    link: 'https://www.etere.com/DocView/9710/Etere-Launches-Web-Application-for-the-Fast-Insertion-of-Metadata.aspx',
    technologies2: 'React, ElectronJS, SCSS, MobX, SOAP, XML, SVN',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
  },
  {
    id: 17,
    count: '11',
    isEmptiness: false,
    title: 'e-commerce косметики та парфюмерії',
    company: 'Sharm Beauty',
    description:
      'Як ментор та тім лід, я веду студентську команду у процесі розробки та модернізації вебсайту https://sharmbeauty.ua, забезпечуючи інтеграцію найкращих практик у дизайні, SEO та розробці за допомогою NextJS та React.',
    technologies2: 'react, NextJs, Redux Toolkit, framer motion, GSAP. Git',
    technologies: [
      {
        id: 1,
        alt: 'React',
        icon: react,
      },
    ],
    link: null,
  },
  {
    id: 18,
    isEmptiness: true,
  },
]

export const projectsData = {
  en: projectsUa,
  uk: projectsUa,
  fr: projectsUa,
}
