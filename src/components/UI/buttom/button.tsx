...
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
          'bg-primary text-primary-foreground shadow hover:bg-primary/90 dark:bg-gray-100/20',
        destructive:
        ...
        success:
          'bg-red-400 text-success-foreground shadow hover:bg-success/90',
        circle:
          ...
          <>
            <span className="text-center duration-500 group-hover:scale-[1.1]">
              ...
              {theme && (
                <Icon
                  name="arrowRight"
                  fill={theme === 'dark' ? '#0B66F5' : '#ffffff'}
                  className="!w-full duration-500 group-hover:scale-[1.05]"
                />
              )}
            </span>
          </>
