    ...
    <>
      <div className="group flex gap-2 text-lg">
        ...
        {!isMobile && (
          <div
            ref={menuRef}
            className="absolute top-[80px] z-10 -ml-[40px] flex max-h-[210px] min-w-[175px] flex-col items-start gap-[6px] overflow-auto rounded-[8px] border-[1px] border-black bg-[#0B66F5] bg-gradient-to-r from-[rgba(255,255,255,0.12)] to-[rgba(255,255,255,0)] p-[8px_12px] backdrop-blur-[12.5px] dark:bg-transparent"
          ...
          >
            {localesData.map((loc) => (
              <div
                ...
                key={loc.name}
                className={`from-white/12 to-transparent/99.21 group flex w-full cursor-pointer items-center gap-2 rounded-lg border-b border-black bg-gradient-to-r px-[12px] py-[8px] normal-case backdrop-blur-[12.5px] duration-300 hover:bg-white hover:dark:bg-stone-500 ${locale === loc.local ? 'font-semibold' : 'font-light'}`}
              >
                {loc.icon && (
                  <Image
      ...
      {isMobile && (
        <div
          ...
          className="absolute -right-5 -top-16 h-screen w-screen rounded-lg border border-black"
          style={{ opacity: 0 }}
        >
          <div
            ref={menuRef}
            className="absolute left-1/2 top-1/2 flex max-h-[210px] min-w-[175px] -translate-x-1/2 -translate-y-1/2 transform flex-col items-start gap-[6px] overflow-auto rounded-[8px] border-[1px] border-black border-stone-500/30 bg-[#0B66F5] p-[8px_12px] backdrop-blur-[12.5px] dark:bg-[linear-gradient(127deg,_rgba(11,_102,_245,_0.50)_49.23%,_rgba(78,_128,_206,_0.15)_83.27%,_rgba(255,_255,_255,_0.00)_102.62%)]"
            style={{ height: 0, opacity: 0 }}
          >
            {localesData.map((loc) => (
              <div
